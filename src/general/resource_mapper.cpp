...
std::pair<const std::vector<VertexData>&, const std::vector<uint32>&> ResourceMapper::loadMesh(
    const std::filesystem::path& path, const vec3u swizzle) const
{
    ...
    NH3D_ASSERT(scene.nodes.size() == 1, "Multi nodes import not supported");

    const tinygltf::Node& node = model.nodes[scene.nodes[0]];

    const tinygltf::Mesh& mesh = model.meshes[node.mesh];

    NH3D_ASSERT(mesh.primitives.size() == 1, "Multi meshes import not supported");
    tinygltf::Primitive primitive = mesh.primitives[0];
    NH3D_ASSERT(primitive.mode == TINYGLTF_MODE_TRIANGLES, "Non-triangle meshes import not supported");

    NH3D_ASSERT(primitive.attributes.find("POSITION") != primitive.attributes.end(), "POSITION missing when loading gltf mesh");
    NH3D_ASSERT(primitive.attributes.find("NORMAL") != primitive.attributes.end(), "NORMAL missing when loading gltf mesh");
    NH3D_ASSERT(primitive.attributes.find("TEXCOORD_0") != primitive.attributes.end(), "TEXCOORD_0 missing when loading gltf mesh");
    ...
    const float* positions
        = reinterpret_cast<const float*>(&(model.buffers[positionView.buffer].data[positionAccessor.byteOffset + positionView.byteOffset]));
